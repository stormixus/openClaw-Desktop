<script lang="ts">
  ...
  // ===== Agent vs Agent (AvA) =====
  type GameMode = 'player' | 'agent' | 'ava';
  let gameMode = $state<GameMode>(restored?.useAgent ? 'agent' : 'player');
  let avaWhiteGw = $state<string>('');
  let avaBlackGw = $state<string>('');
  let avaRunning = $state(false);
  let avaStopped = false;
  const connectedGateways = $derived(store.gateways.filter(g => {
  ...
  const statusText = $derived.by(() => {
    ...
    const t = $ct;
    if (game.isCheckmate()) {
      if (gameMode === 'ava') return game.turn() === 'b' ? 'White wins!' : 'Black wins!';
      return game.turn() === 'b' ? t('win') : t('lose');
    }
    if (game.isStalemate()) return t('stalemate');
    if (game.isDraw()) return t('draw');
    if (aiThinking) {
      if (gameMode === 'ava') return game.turn() === 'w' ? 'White thinking...' : 'Black thinking...';
      return t('ai_thinking');
    }
    if (gameMode === 'ava') return game.turn() === 'w' ? 'White to move' : 'Black to move';
  ...
  function syncBoard() {
    rev++;
    if (!board3d) return;
    board3d.syncPieces(game.board(), { from: lastFrom, to: lastTo });
  ...
  function handleSquareClick(sq: string) {
    if (game.isGameOver() || aiThinking || game.turn() !== 'w' || gameMode === 'ava') return;
  ...
  // ===== Agent vs Agent Loop =====
  async function requestMoveForSide(color: 'w' | 'b'): Promise<boolean> {
    const gwId = color === 'w' ? avaWhiteGw : avaBlackGw;
    const client = getClientById(gwId);
    if (!client) { playRandomMove(); return true; }

    try {
      const prompt = buildChessPrompt(game, $locale, difficulty);
      const sessKey = `chess-ava-${color}-${crypto.randomUUID().slice(0, 8)}`;
      currentSessionKey = sessKey;
      ...
      if (response) {
        ...
        if (san) {
          ...
          const moveObj = verbose.find((m: any) => m.san === san);
          game.move(san);
          if (moveObj) { lastFrom = (moveObj as any).from; lastTo = (moveObj as any).to; }
          const escaped = san.replace(/[+#]/g, '\\$&');
          aiComment = `[${color === 'w' ? 'White' : 'Black'}] ${response.replace(new RegExp(`\\b${escaped}\\b`), '').replace(/^[\s,.:;-]+/, '').trim().slice(0, 200)}`;
          moveList = game.history();
          const turnTokens = Math.round(response.length * 1.3);
          tokensUsed += turnTokens;
          tokenHistory = [...tokenHistory, turnTokens];
  ...
  async function startAva() {
    if (!avaWhiteGw || !avaBlackGw) return;
    avaRunning = true;
    avaStopped = false;

    while (!game.isGameOver() && !avaStopped) {
      aiThinking = true;
      syncBoard();
      const color = game.turn();
      const ok = await requestMoveForSide(color);
      aiThinking = false;
      syncBoard();
      if (!ok) break;
      await delay(500); // brief pause between moves
    }
    avaRunning = false;
  }
...
<div class="chess-3d">
  ...
  <div class="panel">
    ...
    <!-- Agent vs Agent Config -->
    {#if gameMode === 'ava'}
      <div class="panel-section ava-section">
        <span class="section-label">Agent vs Agent</span>
        <div class="ava-row">
          <span class="ava-label">White</span>
          <select class="ava-select" bind:value={avaWhiteGw} disabled={avaRunning}>
        ...
        <div class="ava-row">
          <span class="ava-label">Black</span>
          <select class="ava-select" bind:value={avaBlackGw} disabled={avaRunning}>
            <option value="">Select gateway...</option>
            {#each connectedGateways as gw}
              <option value={gw.id}>{gw.name || gw.url}</option>
            {/each}
        ...
        <div class="ava-controls">
          {#if avaRunning}
            <button class="ava-stop" onclick={stopAva}>Stop</button>
          {:else}
            <button class="ava-start" onclick={startAva} disabled={!avaWhiteGw || !avaBlackGw}>Start Match</button>
          {/if}
        </div>
      </div>
    {/if}
