<script lang="ts">
  ...
  import Go3DViewport from './components/Go3DViewport.svelte';
  import HUD from './components/HUD.svelte';
  import AgentPanel from './components/AgentPanel.svelte';
</script>

<svelte:head>
  <title>{$t('games.module.go3d.title')} | {$t('nav.play')} | {$t('app.title')}</title>
</svelte:head>

<div class="page">
  <div class="header">
    <a href="/play" class="back-link"><ArrowLeft size={16} />{$t('games.back')}</a>
    <h2>{$t('games.module.go3d.title')}</h2>
  </div>
  <div class="layout">
    <div class="left"><Go3DViewport /></div>
    <div class="right">
      <HUD />
      <AgentPanel />
    </div>
