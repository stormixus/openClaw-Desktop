<script lang="ts">
  ...
  import Mines3DViewport from './components/Mines3DViewport.svelte';
  import HUD from './components/HUD.svelte';
  import AgentPanel from './components/AgentPanel.svelte';
</script>

<svelte:head>
  <title>3D Minesweeper | {$t('nav.play')} | {$t('app.title')}</title>
</svelte:head>

<div class="page">
  <div class="header">
    <a href="/play" class="back-link"><ArrowLeft size={16} />{$t('games.back')}</a>
    <h2>3D Minesweeper</h2>
  </div>
  <div class="layout">
    <div class="left"><Mines3DViewport /></div>
